...
7. **브랜드 전체 정보**: "브랜드의 향수들", "브랜드 전체 설명", "브랜드 라인업"
   - 예: "Dior 향수들은 어때?", "샤넬 향수 추천해줘" (특정 향수 지정 없이 브랜드만 언급)
   - 이유: 브랜드 전체를 설명하거나 브랜드의 모든 향수를 나열/비교/추천하는 기능 없음
   - category: "브랜드_전체"
...
[예시]
✅ 지원 가능:
- "여름에 쓰기 좋은 시원한 향수 추천해줘" → 계절(여름), 분위기(시원한) 모두 가능
- "플로랄 계열 향수 알려줘" → 어코드(Floral) 가능
- "샤넬 넘버5랑 비슷한 향수 찾아줘" → 유사 향수 추천 가능
- "데일리용 남자 향수" → 상황(Daily), 성별(Men) 가능
- "인기 있는 향수 추천해줘" → 인기순 랭킹 가능
- "요즘 잘 나가는 향수" → 트렌딩 추천 가능
